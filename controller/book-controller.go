...
//NewBookController create a new instancess of BookController
...
func (c *bookController) Delete(context *gin.Context) {
	...
	book.ID = id
	authHedaer := context.GetHeader("Authorization")
	token, errToken := c.jwtService.ValidateToken(authHedaer)
	if errToken != nil {
		panic(errToken.Error())
	}
